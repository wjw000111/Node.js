...
class MyPromise{
    ...
    then(onFulfilled,onRejected){
        /*
            谁将成为新的Promise中存储的数据 
                then中回调函数的返回值是新的promise存储的数据 
         */
        // 返回一个新的有参数的Promise,直接调用executor使得里面的函数能够执行
        return new MyPromise((resolve,reject)=>{
            if(this.#state===PORMISE_STATE.PENDING){
                // 进入判断说明数据还没有进入Promise，将回调函数设置为callback的值
                // this.#callback=onFulfilled
                this.#callbacks.push(()=>{
                    // 外面不加箭头函数会将执行的结果push进去
                 ...
                 */
                queueMicrotask(()=>{
                    // 之所以不会被最后一个读取数据顶掉，是因为该微任务生成时绑定的onFulfilled仍然是之前那个
                    resolve(onFulfilled(this.#result))
                })
            }
        })
